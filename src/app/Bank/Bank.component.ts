...
export class BankComponent {
	...
	createAssetsBank(): Promise<any> {
		...
		this.bank = {
			$class: 'network.energy.fazezero.Bank',
	...
	updateBank(form: any): Promise<any> {
		...
		this.bank = {
			...
			name: this.name.value,
			coins:
				'resource:network.energy.fazezero.Coins#CO_' + form.get('bankID').value,
			cash:
				'resource:network.energy.fazezero.Cash#CA_' + form.get('bankID').value
		};
		//call serviceBank to update bank, pass bankID of which bank to update as parameter
		return this.serviceBank
			.updateBank(form.get('bankID').value, this.bank)
			.toPromise()
			.then(() => {
				this.errorMessage = null;
			})
