...
export class UtilityCompanyComponent {
	...
	constructor(
		private serviceUtilityCompany: UtilityCompanyService,
		fb: FormBuilder
	) {
		//intialize form
	...
	loadAll(): Promise<any> {
		...
		return this.serviceUtilityCompany
			.getAllUtilityCompanys()
			.toPromise()
			.then(result => {
			...
			.then(() => {
				...
				console.log('ALL UTILITIES', this.allUtilityCompanys);
			});
	}
	...
	createAssetsUtility(): Promise<any> {
		...
		this.coins = {
			$class: 'network.energy.fazezero.Coins',
		...
		this.energy = {
			$class: 'network.energy.fazezero.Energy',
		...
		this.utilityCompany = {
			$class: 'network.energy.fazezero.UtilityCompany',
	...
	updateUtilityCompany(form: any): Promise<any> {
		...
		this.utilityCompany = {
			...
			name: this.name.value,
			coins:
				'resource:network.energy.fazezero.Coins#CO_' +
				form.get('utilityID').value,
			energy:
				'resource:network.energy.fazezero.Energy#EN_' +
				form.get('utilityID').value
		};

		//call serviceUtilityCompany to update utility company, pass utilityID of which utility company to update as parameter
		return this.serviceUtilityCompany
			.updateUtilityCompany(form.get('utilityID').value, this.utilityCompany)
			.toPromise()
			.then(() => {
				this.errorMessage = null;
			})
	...
	deleteUtilityCompany(): Promise<any> {
		...
		return this.serviceUtilityCompany
			.deleteUtilityCompany(this.currentId)
			.toPromise()
			.then(() => {
				this.errorMessage = null;
