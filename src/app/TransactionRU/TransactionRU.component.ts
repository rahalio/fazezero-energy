...
export class TransactionRUComponent {
	...
	constructor(
		private serviceTransaction: TransactionRUService,
		fb: FormBuilder
	) {
		//intialize form
	...
	execute(form: any): Promise<any> {
		...
		this.energyToCoinsObj = {
			$class: 'network.energy.fazezero.EnergyToCoins',
					...
					this.serviceTransaction
						...
						.then(checkConsumerCoins => {
							...
							if (checkConsumerCoins) {
								...
								this.serviceTransaction
									.energyToCoins(this.energyToCoinsObj)
									.toPromise()
									.then(result => {
										this.errorMessage = null;
