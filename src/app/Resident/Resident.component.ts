...
export class ResidentComponent {
	...
	loadAll(): Promise<any> {
		...
		return this.serviceResident
			...
			.then(() => {
				...
				console.log('ALL RESIDENTS', residentList);
			});
	}
	...
	createAssetsResident(): Promise<any> {
		...
		this.cash = {
			$class: 'network.energy.fazezero.Cash',
		...
		this.resident = {
			$class: 'network.energy.fazezero.Resident',
	...
	updateResident(form: any): Promise<any> {
		...
		this.resident = {
			...
			coins:
				...
				form.get('residentID').value,
			cash:
				'resource:network.energy.fazezero.Cash#CA_' +
				form.get('residentID').value,
			energy:
		...
		return this.serviceResident
			.updateResident(form.get('residentID').value, this.resident)
			.toPromise()
			.then(() => {
				this.errorMessage = null;
			})
